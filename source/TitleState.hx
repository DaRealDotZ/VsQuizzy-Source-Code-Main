...
class TitleState extends MusicBeatState
{
	...
	var introItem:Int = FlxG.random.int(1, 100);
	...
	function startIntro()
	{
		...
		amogus = new FlxSprite(0, FlxG.height * 0.4).loadGraphic(Paths.image('amogus'));
		amogus.setGraphicSize(Std.int(amogus.width * 0.55));
		amogus.updateHitbox();
		amogus.screenCenter(X);
		amogus.antialiasing = ClientPrefs.globalAntialiasing;

		pac = new FlxSprite(0, FlxG.height * 0.4).loadGraphic(Paths.image('sad'));
		pac.setGraphicSize(Std.int(pac.width * 0.55));
		pac.updateHitbox();
		pac.screenCenter(X);
		pac.antialiasing = ClientPrefs.globalAntialiasing;

		if (introItem == 10)
		{ // AMOGUSSUSUSUSUSUUSSSSSSSSSSSSSSSSSSSSSSSSSS?!??!??!??!??!?!??!?!??!?!?!!??!!?!??!?!?!?!?!?!
			add(amogus);
			amogus.visible = false;
		} else {
			if (introItem == 20)
			{ // pac gorilla is coming for us run away AHHHHHHHHHHHHHHHHHHHHHHH!
				add(pac);
				pac.visible = false;
			} else {
				add(ngSpr);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					//createCoolText(['Vs Quizzy by'], 15);
					#else
					createCoolText(['ninjamuffin99', 'phantomArcade', 'kawaisprite', 'evilsk8er']);
					#end
				case 3:
					#if PSYCH_WATERMARKS
					createCoolText(['PacTV', 'DotZ', 'mrFloras', 'Noix8bit', 'Timson']);
					#else
					addMoreText('present');
					#end
				case 4:
				...
				case 7:
					if (introItem == 10)
					{
						addMoreText('amogus', -40);
					} else {
						if (introItem == 20)
						{
                            addMoreText('death', -40); // wanna work on my fnf mod?
						} else {
							addMoreText('newgrounds', -40);
						}
					}
					amogus.visible = true;
					pac.visible = true; // HELPPPPPPPP
					ngSpr.visible = true;
				case 8:
					deleteCoolText();
					amogus.visible = false;
					pac.visible = false; // PLEASE
					ngSpr.visible = false;
				case 9:
